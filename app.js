...
io.on('connection', async (socket) => {
    ...
    socket.on('client:eraserequest',async(userid)=>{
        ...
        const results = await pool.promise().query('UPDATE chatters set background_picture = "" WHERE id = ?', [userid])
        sessionweb.userbg = ""
        sessionweb.save()
    ...
    socket.on('client:changetheme', async (data) => {
        const results = await pool.promise().query('UPDATE chatters set theme = ? WHERE accesscode = ?', [data[0],data[1]])
    ...
    socket.on('client:changebg', async (data) => {
        const results = await pool.promise().query('UPDATE chatters set background_picture = ? WHERE accesscode = ?', [data[0],data[1]])
        let base64String = '';
